...
import { client } from "../utilities/state";
...
const CallConnector = ({
    ...
    const produce = async () => {
        if (!screensharing) {
            ...
            const vid = await navigator.mediaDevices.getDisplayMedia({
                video: true,
            });
    ...
    return (
        <div class="flex flex-col w-full py-4 border-b">
            <div
                class={`self-center text-sm space-x-3${
                    connecting
                        ? " text-yellow-400"
                        : connected
                        ? " text-green-400"
            ...
            <div
                id="video-list"
                class="w-3/4 self-center flex flex-wrap justify-center"
            >
                {callMembers &&
                    callMembers.map((m) => (
                        <div
                            key={m[0]}
                            class="user rounded-md bg-violet-400 w-1/3 m-2 p-4 border relative"
                        >
                            <br />
                            <div class="bottom-4 absolute text-white">
                                {client.client.users.get(m[1]) ??
                                    "Unknown user"}
                            </div>
                        </div>
                    ))}
                ...
                {videoTracks &&
                    videoTracks.map((t) => (
                        <Video
                            class="w-1/3 m-2 rounded-md border"
                            srcObject={t.stream}
                            key={t.consumerId}
                        />
                    ))}
            ...
            <div class="self-center flex flex-wrap justify-center">
                <button
                    onClick={connectCall}
                    class={`rounded-md text-white px-4 m-2 tooltip h-20${
                ...
                >
                    <Call24Filled />
                    <p class="text-sm">Connect call</p>
                </button>
                <button
                    onClick={produce}
                    class={`rounded-md text-white px-4 m-2 tooltip h-20${
                        !connected ? " bg-gray-400" : " bg-violet-400"
                    }`}
                    disabled={!connected}
                >
                    {screensharing ? (
                        <>
                            <ShareScreenStop24Filled />
                            <p class="text-sm">Stop sharing</p>
                        </>
                    ) : (
                        <>
                            <ShareScreenStart24Filled />
                            <p class="text-sm">Share screen</p>
                        </>
                    )}
                </button>
                <button
                ...
                >
                    {unmuted ? (
                        <>
                            <Mic24Filled />
                            <p class="text-sm">Mute</p>
                        </>
                    ) : (
                        <>
                            <MicOff24Filled />
                            <p class="text-sm">Unmute</p>
                        </>
                    )}
                </button>
                <button
