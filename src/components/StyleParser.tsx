...
const StyleParser = ({
    ...
    showModalDialog: (
        title: string,
        message: string,
        buttons: { text: string; id: string; primary?: boolean }[],
        ...
        if (node.tagName === "A") {
            return (
                <a
                    ...
                    onClick={(e) =>
                        handleLink(e, node.getAttribute("href") ?? undefined)
                    }
                >
                    {children}
                </a>
    ...
    const parse = (raw: string): string => {
        ...
        const bold = (match: string, s: string) =>
            typeof s === "number" ? "" : `<b>${s}</b>`;
        const boldRegex = /\*\*(.*?)\*\*/gs;
        const italic = (match: string, s: string) =>
            typeof s === "number" ? "" : `<i>${s}</i>`;
        const italicRegex = /\*(.*?)\*/gs;
        const code = (match: string, s: string) =>
            typeof s === "number"
                ? ""
                : `<code class="bg-gray-700">${s}</code>`;
        const codeRegex = /`(.*?)`/gs;
        const underline = (match: string, s: string) =>
            typeof s === "number" ? "" : `<u>${s}</u>`;
        const underlineRegex = /__(.*?)__/gs;
        const strikethrough = (match: string, s: string) =>
            typeof s === "number" ? "" : `<s>${s}</s>`;
        const strikethroughRegex = /~~(.*?)~~/gs;
        const multilineCode = (match: string, s: string) =>
            typeof s === "number" ? "" : `<pre>${s}</pre>`;
        const multilineCodeRegex = /```(.*?)```/gs;
        const emphasis = (match: string, s: string) =>
            typeof s === "number" ? "" : `<em>${s}</em>`;
        const emphasisRegex = /_(.*?)_/gs;
        const spoiler = (match: string, s: string) =>
            typeof s === "number" ? "" : `<span class="spoiler">${s}</span>`;
        const spoilerRegex = /\|\|(.*?)\|\|/gs;

        const link = (match: string, s: string) =>
            typeof s === "number" ? "" : `<a href="${encodeURI(s)}">${s}</a>`;
        const linkRegex =
